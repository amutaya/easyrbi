...
2. rbi() return stream hydrology data for given USGS sites with the calculated RBI value for a specified period.
...
```
4. This returns summary statistics for all USGS sites given in the rbiWy_dfAll dataframe.
```{r, eval=TRUE}
trends() %>% 
  head()
```
